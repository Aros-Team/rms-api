...
import aros.services.rms.core.auth.domain.RefreshTokenId;
...
public class AuthService implements LoginUseCase, VerifyTwoFactorUseCase {
    ...
    public AuthResult authenticate(Credentials credentials) throws InvalidCredentialsException {
        ...
        if (!passwordPort.validate(user.getPassword(), credentials.password())) {
            throw new InvalidCredentialsException();
        }

        Optional<Device> deviceOpt = devicePort.findByUserIdAndHash(user.getId(), credentials.deviceHash());
    ...
    public AuthResult verify(TwoFactorCredentials credentials) throws InvalidCredentialsException {
        User user = userPort.findByEmail(credentials.username().value())
            .orElseThrow(InvalidCredentialsException::new);

        String codeHash = hashServicePort.hash(credentials.code());

        TwoFactorCode twoFactorCode = tfaPort.findValidCode(user.getId(), codeHash)
            .orElseThrow(InvalidCredentialsException::new);
    ...
    private AuthResult generateAuthResult(AuthResultType type, String username, User user) {
        if (type.equals(AuthResultType.TFA_REQUIRED)) {
            ...
            return new AuthResult(AuthResultType.TFA_REQUIRED, username, tokenPort.generateTFAToken(user));
        }
        ...
        RefreshToken refreshToken = new RefreshToken(
            RefreshTokenId.of(System.currentTimeMillis()),
            user.getId(),
            refreshHash,
            expiresAt,
            Instant.now()
