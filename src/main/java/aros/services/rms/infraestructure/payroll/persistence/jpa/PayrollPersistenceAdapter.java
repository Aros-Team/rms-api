...
public class PayrollPersistenceAdapter implements PayrollRepositoryPort {
  ...
  public Payroll save(Payroll payroll) {
    return payrollMapper.toDomain(
        internal.save(payrollMapper.toEntity(payroll)), currencyProvider.getCurrency());
  }
  ...
  public Optional<Payroll> findById(Long id) {
    return internal
        .findById(id)
        .map(e -> payrollMapper.toDomain(e, currencyProvider.getCurrency()));
  }
  ...
  public List<Payroll> findAll() {
    return internal.findAll().stream()
        .map(e -> payrollMapper.toDomain(e, currencyProvider.getCurrency()))
        .toList();
  }
  ...
  public List<Payroll> findByUserId(Long userId) {
    return internal.findByUserId(userId).stream()
        .map(e -> payrollMapper.toDomain(e, currencyProvider.getCurrency()))
        .toList();
  }
