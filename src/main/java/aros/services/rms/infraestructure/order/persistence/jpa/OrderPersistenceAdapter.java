...
public class OrderPersistenceAdapter implements OrderRepositoryPort {
    ...
    public Order save(Order order) {
        if (order.getId() != null) {
            ...
            if (existingOpt.isPresent()) {
                ...
                aros.services.rms.infraestructure.order.persistence.Order mapped = orderMapper.toEntity(order);
                ...
                aros.services.rms.infraestructure.order.persistence.Order savedEntity = orderRepository.save(existing);
    ...
    public Optional<Order> findFirstByStatusOrderByDateAsc(OrderStatus status) {
        ...
        return orderRepository.findFirstByStatusOrderByDateAsc(infraStatus)
                .map(orderMapper::toDomain);
    }
    ...
    @Override
    public List<Order> findByStatusAndDateBetween(OrderStatus status, LocalDateTime startDate, LocalDateTime endDate) {
        aros.services.rms.infraestructure.order.persistence.OrderStatus infraStatus =
                aros.services.rms.infraestructure.order.persistence.OrderStatus.valueOf(status.name());
